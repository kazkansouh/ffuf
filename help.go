...
func Usage() {
	u_http := UsageSection{
		...
		Hidden:        false,
		ExpectedFlags: []string{"H", "X", "b", "d", "r", "u", "recursion", "recursion-depth", "replay-proxy", "timeout", "ignore-body", "x", "maxtries", "retrydelay"},
	}
	u_general := UsageSection{
		...
		Flags:         make([]UsageFlag, 0),
		Hidden:        false,
		ExpectedFlags: []string{"ac", "acc", "c", "maxtime", "maxtime-job", "p", "s", "sa", "se", "sf", "t", "v", "V", "banner", "dont-check-content-type", "dont-search-backups"},
	}
